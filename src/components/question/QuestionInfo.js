...
const QuestionInfo = ({ question, rating, onRate = f => f }) => (
  <Container className="question-information">
    ...
    {question.tags && question.tags.concat(question.topics.map(topic => topic.name)).length > 0 ? (
      <Row className="c-question__row-info">
        <Col className="info-label" sm="4" xs="4">
          Tags
        </Col>
        <Col sm="8" xs="8">
          <TagList list={question.tags.concat(question.topics.map(topic => topic.name))} styleTag="question-info  tag-name" />
        </Col>
      </Row>
    ) : ' '}
